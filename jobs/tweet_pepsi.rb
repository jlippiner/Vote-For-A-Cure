...
class TweetPepsi
  ...
  def perform
    refresh_searches('phrase=vote+to+give+this+idea&tag=pepsirefresh','pepsi')
    refresh_searches('phrase=I+clicked+to&ref=care2','care2_i_clicked')
    ...
    send_replies(endsmadotcom,'pepsi')
    send_replies(endsmadotcom,'care2_i_clicked')

    dwrite("TweetPepsi: sending from voteforacure")
    send_replies(voteforacure,'pepsi')
    send_replies(voteforacure,'care2_i_clicked')
  end
